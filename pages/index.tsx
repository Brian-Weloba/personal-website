            ...
            <main className="w-full overflow-y-scroll">
                ...
                <div className="h-fit mt-8 md:mx-4">
                    <div className="flex font-poppins font-bold">
                        <div
                            ...
                            className={`w-1/3 py-4 border-t-2 border-l-2 border-r-2 rounded-t-xl justify-center cursor-pointer flex ${activeTab === "frontend" ? "active-tab" : "inactive-tab"
                                }`}>
                            FRONT-END
                        </div>
                        <div
                            onClick={() => handleTabChange("backend")}
                            className={`w-1/3 py-4 border-t-2 border-l-2 border-r-2 rounded-t-xl bdr justify-center cursor-pointer flex ${activeTab === "backend" ? "active-tab" : "inactive-tab"
                                }`}>
                            BACK-END
                        </div>
                        <div
                            onClick={() => handleTabChange("software")}
                            className={`w-1/3 py-4 border-t-2 border-l-2 border-r-2 rounded-t-xl bdr justify-center flex cursor-pointer ${activeTab === "software" ? "active-tab" : "inactive-tab"
                                }`}>
                            SOFTWARE
                        </div>
                    </div>
                ...
                <div className="flex justify-center h-fit bdr mt-4 ">
                    <div
                        ...
                        onClick={handleClick}
                        className={`cursor-pointer w-fit flex h-fit bg-transparent frg-text m-4 bkg-inv border-2 rounded-2xl overflow-clip ${isHovered ? "bdr-inv" : "bdr"
                            }`}>
                        <div
                            className={`h-auto flex flex-col justify-around  p-4 ${isHovered ? "bkg-inv" : "bkg"
                                }`}>
                            <FaSpotify
                                className={`text-6xl mx-auto ${isHovered ? "text-green-400" : ""
                                    }`}
                            />
                            <h1
                                className={` text-base text-center font-semibold font-poppins mx-auto  ${isHovered ? "frg-text-inv" : "fgr-text"
                                    }`}>
                                NOW PLAYING
                            </h1>
                        </div>
                        <div
                            className={`p-2  flex justify-center ${isHovered ? "" : "bkg bdr-2"
                                }`}>
                            <DynamicImage />
                        </div>
                    </div>
